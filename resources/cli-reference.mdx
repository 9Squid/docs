...
Every command, generated from the CLI itself. `REQUIRED` flags and positional `<args>` must be provided. Global flags (`--json`, `--table`, `--non-interactive`, `--yes`, `--debug`, `--watch`, `--api-key`/`--api-secret`) apply to every command — see the [CLI Developer Guide](/resources/cli).
...
Sandbox `tax_id` test paths (deterministic KYC outcomes):
  * ending `0001` → approved
  * ending `0002` → rejected
  * ending `0003` → manual_review
